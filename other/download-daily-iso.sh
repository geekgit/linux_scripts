...
LinkSums=$(grep -Po '(?<=href=")[^"]*' "$UbuntuFile" | grep -v "http" | grep "SUMS" | grep -v "zsync" | grep -v "metalink" | sort | uniq)
LinkIso=$(grep -Po '(?<=href=")[^"]*' "$UbuntuFile" | grep -v "http" | grep "iso" | grep -v "zsync" | grep -v "metalink" | sort | uniq)
for Sum in $LinkSums
do
